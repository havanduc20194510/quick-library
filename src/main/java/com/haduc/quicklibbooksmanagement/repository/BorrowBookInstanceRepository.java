...
public interface BorrowBookInstanceRepository extends JpaRepository<BorrowBookInstance, Long> {
    ...
    @Query(value = "SELECT b.id as book_id, b.title as title, b.publisher as publisher, b.cover_image_url as book_image, c.id as category_id, c.name as category_name, c.parent_category_id as parent_category_id, l.id as library_id, l.name as library_name, br.id as borrow_request_id, br.created_at as request_date, br.borrow_date as borrow_date, br.request_due_date as request_due_date, br.status as status_request, brd.id as borrow_book_instance_id FROM borrow_request br JOIN borrow_book_instances brd ON br.id = brd.borrow_id JOIN book_instances lb ON brd.book_instance_id = lb.id JOIN books b ON lb.book_id = b.id JOIN libraries l ON lb.library_id = l.id JOIN categories c ON b.category_id = c.id  WHERE br.id = :requestId", nativeQuery = true)
    List<BorrowRequestBookInfo> findBooksByBorrowRequestId(Long requestId);
    List<BorrowBookInstance> findByBorrowRequest_Id(Long borrowRequestId);
    @Query(value = "SELECT b.id as book_id, b.title as title, b.publisher as publisher, b.cover_image_url as book_image, c.id as category_id, c.name as category_name, c.parent_category_id as parent_category_id, l.id as library_id, l.name as library_name, br.id as borrow_request_id, br.created_at as request_date, br.borrow_date as borrow_date, br.request_due_date as request_due_date, br.status as status_request, brd.id as borrow_book_instance_id FROM borrow_request br JOIN borrow_book_instances brd ON br.id = brd.borrow_id JOIN book_instances lb ON brd.book_instance_id = lb.id JOIN books b ON lb.book_id = b.id JOIN libraries l ON lb.library_id = l.id JOIN categories c ON b.category_id = c.id  WHERE br.user_id = :userId AND br.status NOT IN ('UNSENT', 'REQUESTED')", nativeQuery = true)
    List<BorrowRequestBookInfo> findBorrowBookByUserId(Long userId);
}
