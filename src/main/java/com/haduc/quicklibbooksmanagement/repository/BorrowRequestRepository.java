...
public interface BorrowRequestRepository extends JpaRepository<BorrowRequest, Long> {
    ...
    @Query(value = "SELECT br.id as request_id, br.user_id as user_id, u.username as user_name, br.status as status, br.borrow_date as borrow_date, br.request_due_date as request_due_date, COUNT(brb.book_instance_id) as book_number FROM borrow_book_instances brb JOIN borrow_request br ON brb.borrow_id = br.id JOIN users u ON br.user_id = u.id GROUP BY (br.id,user_name) ORDER BY borrow_date DESC", nativeQuery = true)
    List<BorrowRequestItem> findAllBorrowRequestItems();
